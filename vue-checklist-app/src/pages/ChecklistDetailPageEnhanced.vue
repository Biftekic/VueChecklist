      ...
      <div v-if="checklist && !loading" class="checklist-detail">
        ...
        <v-window v-model="activeTab">
          ...
          <v-window-item value="route">
            <RouteVisualization
              v-if="checklist?.selectedTasks || checklist?.tasks"
              :tasks="checklist.selectedTasks || checklist.tasks"
...
import InspectionChecklist from '@/components/quality/InspectionChecklist.vue'
...
const totalTasks = computed(() => {
  const tasks = checklist.value?.selectedTasks || checklist.value?.tasks || []
  return tasks.length
})
...
const loadChecklist = async () => {
  ...
  try {
    ...
    if (!checklist.value) {
      ...
      if (!checklist.value.tasks && checklist.value.selectedTasks) {
        checklist.value.tasks = checklist.value.selectedTasks
      }
...
const applyRoute = async (route) => {
  // Apply the optimized route to the checklist
  checklist.value.optimizedRoute = route
  await checklistsStore.updateChecklist(checklist.value.id, checklist.value)
  appStore.showSnackbar('Route applied successfully', 'success')
}

const exportRoute = (routeData) => {
  // Export route data as JSON
  const dataStr = JSON.stringify(routeData, null, 2)
  const dataUri = 'data:application/json;charset=utf-8,'+ encodeURIComponent(dataStr)
  
  const exportFileDefaultName = `route_${checklist.value.name}_${new Date().toISOString().split('T')[0]}.json`
  ...
  appStore.showSnackbar('Route exported successfully', 'success')
}

const updateTeam = async (team) => {
  checklist.value.team = team
  await checklistsStore.updateChecklist(checklist.value.id, checklist.value)
  appStore.showSnackbar('Team updated successfully', 'success')
}

const handleAssignmentsChanged = async (data) => {
  // Save assignment changes
  checklist.value.teamAssignments = data.assignments
  await checklistsStore.updateChecklist(checklist.value.id, checklist.value)
}

const handleInspectionComplete = async (inspectionData) => {
  // Update checklist with quality score
  checklist.value.qualityScore = inspectionData.qualityScore
  checklist.value.lastInspection = new Date().toISOString()
  checklist.value.inspectionData = inspectionData
  
  // Update status if inspection passed
  if (inspectionData.qualityScore >= 75) {
    checklist.value.status = 'completed'
  }
  
  await checklistsStore.updateChecklist(checklist.value.id, checklist.value)
  appStore.showSnackbar(`Inspection completed with ${inspectionData.qualityScore}% quality score`, 'success')
}
